...
首先，采用LightGBM结合SHAP可解释性方法，对库水位、降雨量、地下水位等多源监测数据进行特征重要性分析，定性揭示了历史位移累积与库水位滞后特征对滑坡位移预测的主控作用，降雨、地下水位次之，气象特征（温度、露点、湿度）贡献最小，验证了滑坡变形的累积驱动与水文气象耦合机制假设。

其次，基于多监测点空间运动学输入，构建了长短期记忆网络（LSTM）时序预测模型，对MJ9、MJ1、MJ3三个代表性监测点分别进行50次独立随机初始化训练。测试集结果表明，LSTM模型在MJ1点取得了典型的预测性能：50次独立运行的平均决定系数$R^2=0.7916$、平均均方根误差RMSE$=$5.55~mm、相对误差约0.56\%，达到工程应用精度要求；MJ9与MJ3两点在RMSE与相对误差指标上同样处于工程可接受区间（相对误差分别约2.9\%与1.0\%）。
...
\noindent\hspace{2em}First, LightGBM combined with the SHAP interpretability method is employed to analyze feature importance using multi-source monitoring data including reservoir water level, rainfall, and groundwater level. The results qualitatively reveal that historical displacement accumulation and lagged reservoir water level features are the dominant controlling factors of landslide displacement prediction, followed by rainfall and groundwater level, while meteorological variables (temperature, dew point, humidity) contribute least. These findings validate the hypothesis of a cumulative driving mechanism coupled with hydro-meteorological effects.

\noindent\hspace{2em}Second, a Long Short-Term Memory (LSTM) time-series prediction model is constructed based on multi-monitoring-point spatial kinematic inputs, and 50 independent random-initialization training runs are performed on each of the three representative monitoring points MJ9, MJ1 and MJ3. Test set results show that the LSTM model achieves representative accuracy at MJ1: a mean coefficient of determination $R^2 = 0.7916$, a mean root mean square error RMSE of 5.55~mm, and a relative error of only 0.56\% over 50 runs, meeting engineering application accuracy requirements; MJ9 and MJ3 likewise stay within engineering-acceptable relative-error ranges (approximately 2.9\% and 1.0\%).
...
\begin{enumerate}
    \item 以多监测点历史累积位移序列作为空间运动学输入特征，构建LSTM时序预测模型，利用滑坡体的空间协同变形信息提升目标监测点的位移预测精度；
    \item 对MJ9、MJ1、MJ3三个代表性监测点分别进行50次独立随机初始化训练，收集预测结果的统计分布；
    \item 以$R^2$与RMSE（50次独立运行的逐次算术平均）评估各点点预测精度与稳定性，并结合相对误差进行工程意义上的综合评价。
\end{enumerate}

\paragraph{四、基于$V_0$阈值的四级动态预警体系}

\begin{enumerate}
    \item 对各监测点匀速变形阶段的位移速率序列，按$V_0 = 1.5\bar V + 2\sigma$确定基准速率，并按$V_0/5V_0/10V_0$递进设定绿/黄/橙/红四级阈值；
    \item 将50次独立LSTM预测得到的月速率分布映射到四个等级区间，直接统计每一天四级预警的发生概率，并取概率最高者作为当日预警等级；
    \item 以测试集盲测下的特异性、误报率以及训练期阶跃段的黄色预警触发情况综合评估预警体系，分析方法在误报控制与阶跃响应两方面的表现。
\end{enumerate}
