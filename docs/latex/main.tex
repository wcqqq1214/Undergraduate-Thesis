...
在第二章中，本研究通过LightGBM模型结合SHAP方法，系统地识别并量化了影响藕塘滑坡位移变形的关键触发因子，包括降雨、库水位、地下水位及气象因素等。这些特征的筛选为本章的位移预测奠定了特征基础。然而，第二章的工作主要聚焦于特征重要性分析，尚未构建位移预测模型。

水库滑坡的位移演化过程具有复杂的非线性时变特性。在库水位变化和降雨等外部触发因素作用下，滑坡体会产生突发性的位移增量，而在其他时段则相对稳定。这种复杂的时序特征使得传统线性模型难以准确刻画滑坡的变形规律，需要能够捕捉长期依赖关系的时序预测模型。

为此，本章引入LSTM和GRU两种主流循环神经网络。LSTM通过门控机制捕捉长期依赖，适合复杂时序预测；GRU是LSTM的简化版本，参数更少，训练更快。通过对比验证两种模型的性能，评估模型复杂度与预测精度的权衡。

此外，传统点预测无法量化不确定性，在工程应用中存在局限性。为了提供更全面的预测信息，本章采用50次运行集成方法构建概率预测框架。对同一模型进行50次独立训练，每次使用不同的随机种子初始化参数，收集50次预测结果形成统计分布。该方法不仅能够输出位移的期望值，还能同时给出不同置信水平下的预测区间，为滑坡灾害的风险评估与预警决策提供更加全面的信息支持。

本章以藕塘滑坡MJ9监测点为研究对象，聚焦单监测点位移预测，验证所提方法的有效性与可靠性，为后续章节的空间预测和预警研究奠定基础。


\subsection{方法原理}

本章提出的LSTM/GRU概率预测模型主要包括两个核心模块：循环神经网络时序预测模块和概率预测模块。模型直接建立触发因子到累积位移的映射关系，不进行位移分解。将第二章筛选出的重要触发因子（降雨、库水位、地下水位等）作为输入，分别构建LSTM和GRU时序预测模型，直接预测未来时刻的累积位移；通过50次独立运行获得预测的统计分布，输出不同置信水平下的预测区间，实现位移的概率预测。
...
在本研究中，LSTM模型的输入特征包括第二章通过SHAP方法筛选出的重要触发因子（降雨量、库水位、地下水位、平均温度、最低温度、最高温度、露点、相对湿度）的历史序列。模型的输出为未来时刻的累积位移预测值。通过多层LSTM结构的堆叠，模型能够学习更加复杂的时序特征表示，从而提高预测精度。

\subsubsection{门控循环单元（GRU）}

门控循环单元（Gated Recurrent Unit, GRU）是由Cho等人于2014年提出的一种简化的循环神经网络结构，可以看作是LSTM的变体。GRU通过合并LSTM中的遗忘门和输入门为单一的更新门，并将记忆单元和隐藏状态合并，从而减少了模型参数数量，提高了训练效率。尽管结构更简单，GRU在许多序列建模任务中表现出与LSTM相当甚至更优的性能。

GRU的核心结构包括两个门控单元：更新门和重置门。在时刻$t$，GRU单元接收当前输入$\mathbf{x}_t$和上一时刻的隐藏状态$\mathbf{h}_{t-1}$，通过以下计算过程更新隐藏状态$\mathbf{h}_t$：
...
（3）候选隐藏状态：计算当前时刻的候选激活值
\begin{equation}
\tilde{\mathbf{h}}_t = \tanh(\mathbf{W}_h \cdot [\mathbf{r}_t \odot \mathbf{h}_{t-1}, \mathbf{x}_t] + \mathbf{b}_h)
\label{eq:candidate-hidden}
\end{equation}

（4）隐藏状态更新：结合更新门的输出更新隐藏状态
\begin{equation}
\mathbf{h}_t = (1 - \mathbf{z}_t) \odot \mathbf{h}_{t-1} + \mathbf{z}_t \odot \tilde{\mathbf{h}}_t
\label{eq:gru-hidden-update}
\end{equation}

其中，$\sigma$为Sigmoid激活函数，$\tanh$为双曲正切激活函数，$\odot$表示逐元素乘法，$\mathbf{W}$和$\mathbf{b}$分别为权重矩阵和偏置向量。

相较于LSTM，GRU的主要优势在于：（1）参数更少，约为LSTM的75\%，训练速度更快；（2）结构更简洁，更易于理解和实现；（3）在许多任务中表现出与LSTM相当的性能。本研究通过对比LSTM和GRU两种模型，评估模型复杂度与预测精度的权衡，为滑坡位移预测提供模型选择依据。

在本研究中，GRU模型与LSTM模型采用相同的输入特征和输出目标，即以第二章筛选出的重要触发因子作为输入，直接预测未来时刻的累积位移。通过对比两种模型的预测性能，可以评估模型复杂度对预测精度的影响。

\subsubsection{基于多次运行的概率预测}

传统的点预测方法仅能输出位移的单一预测值，无法量化预测结果的不确定性。为了提供更加全面的预测信息，本研究采用多次独立运行的方法构建概率预测框架。具体而言，对于LSTM和GRU模型，分别进行50次独立训练和预测，每次使用不同的随机种子初始化模型参数。通过收集50次运行的预测结果，可以获得预测值的统计分布，从而计算出不同置信水平下的预测区间。
...
本研究以藕塘滑坡的MJ9监测点为主要研究对象，采用2010年至2023年的长期监测数据，包括累积位移、降雨量、库水位、地下水位等。数据采样频率为日尺度。

输入特征基于第二章SHAP分析筛选的8个重要触发因子：
\begin{enumerate}
\item 降雨量（mm）
\item 库水位（m）
\item 地下水位（m）
...
输出目标为累积位移（mm），模型直接预测累积位移，不进行趋势/周期分解。
...
\item 时间窗口：使用过去T天的数据预测未来1天（T根据实验确定）；
\item 数据集划分：按时间顺序将数据划分为训练集（80\%）和测试集（20\%），保持时间序列的连续性。
\end{enumerate}

\subsubsection{网络结构和超参数设置}

LSTM和GRU模型采用TensorFlow/Keras深度学习框架实现。两个模型采用相同的网络结构和训练参数，以确保对比的公平性。
...
\subsubsection{LSTM预测结果}

基于50次独立训练运行，LSTM模型在MJ9监测点上取得了良好的预测性能。表3-2展示了LSTM模型的点预测性能，其中预测值为50次运行的均值。
...
从表中可以看出，LSTM模型在测试集上取得了优异的预测性能。测试集$R^2$达到0.9139，表明模型能够解释91.39\%的位移变化，具有良好的泛化能力。RMSE为4.99 mm，相对于累积位移量（约500-600 mm），预测误差率约为1\%，达到了工程应用的精度要求。训练集和测试集性能差距不大，未出现明显过拟合现象。

\subsubsection{GRU预测结果}

表3-3展示了GRU模型的点预测性能。GRU作为LSTM的简化变体，在滑坡位移预测任务上同样表现出色。

\begin{table}[htbp]
\centering
\caption{GRU模型点预测性能（基于50次运行均值）}
\label{tab:gru-point-prediction}
\begin{tabular}{lcc}
\toprule
数据集 & $R^2$ & RMSE (mm) \\
\midrule
...
从表中可以看出，GRU模型在测试集上同样取得了优异的预测性能。测试集$R^2$达到0.9115，与LSTM模型的0.9139非常接近，表明GRU模型具有与LSTM相当的泛化能力。测试集RMSE为5.73 mm，略高于LSTM的4.99 mm，但差异仅为0.74 mm，相对于累积位移量（约500-600 mm）可忽略不计。值得注意的是，GRU模型在训练集上的表现甚至优于LSTM（$R^2$=0.9968 vs 0.9907，RMSE=12.40 mm vs 16.96 mm），这可能得益于GRU更简洁的门控结构减少了过拟合风险。总体而言，GRU模型在保持预测精度的同时，参数量约为LSTM的75\%，训练速度更快，在实际工程应用中具有更高的效率优势。

\subsubsection{LSTM与GRU对比分析}

表3-4系统对比了LSTM和GRU两个模型的概率预测统计特征。基于50次独立训练运行，两个模型的预测分布呈现高度相似的特征。

\begin{table}[htbp]
\centering
\caption{LSTM与GRU概率预测统计对比}
\label{tab:lstm-gru-comparison}
\begin{tabular}{lcc}
\toprule
统计量 & LSTM & GRU \\
\midrule
平均预测值 (mm) & 578.64 & 581.47 \\
平均标准差 (mm) & 407.96 & 409.91 \\
最大标准差 (mm) & 466.65 & 472.92 \\
最小标准差 (mm) & 308.27 & 296.88 \\
\bottomrule
\end{tabular}
\end{table}

从表中可以看出，LSTM和GRU的平均预测值非常接近，相差不到3 mm（约0.5\%）。两个模型的预测不确定性（标准差）也相似，平均标准差均在400 mm左右。这表明在滑坡位移预测任务上，LSTM和GRU的性能相当，均可作为可靠的预测工具。

图\ref{fig:lstm-gru-comparison}展示了LSTM和GRU模型的概率预测结果对比。上图为LSTM模型的预测结果，下图为GRU模型的预测结果。两个子图均显示了均值预测（实线）、50\%置信区间（深色阴影，25\%-75\%分位数）和90\%置信区间（浅色阴影，5\%-95\%分位数）。从图中可以看出，两个模型的预测曲线走势高度一致，置信区间宽度也相近，直观地验证了两者性能的相似性。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/chapter3/lstm_gru_comparison.pdf}
\caption{LSTM与GRU模型概率预测结果对比（上：LSTM，下：GRU）}
\label{fig:lstm-gru-comparison}
\end{figure}

为了更直观地对比两个模型，图\ref{fig:lstm-gru-combined}将LSTM和GRU的预测结果绘制在同一坐标系中。图中蓝色曲线为LSTM的均值预测和90\%置信区间（5\%-95\%分位数），红色曲线为GRU的对应结果。从图中可以看出，两个模型的预测曲线高度重合，置信区间也高度重叠，进一步验证了两者性能的一致性。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/chapter3/lstm_gru_combined.pdf}
\caption{LSTM与GRU概率预测综合对比}
\label{fig:lstm-gru-combined}
\end{figure}

为了更深入地分析两个模型的不确定性特征，图\ref{fig:uncertainty-comparison}展示了LSTM和GRU模型预测标准差随时间的变化。从图中可以看出，两条曲线走势高度一致，在位移变化剧烈的时期（如2018年中期），两个模型的不确定性均显著增大，标准差达到最大值（LSTM为466.65 mm，GRU为472.92 mm）。在位移相对平稳的时期，不确定性相对较小。这种不确定性的时变特征符合滑坡位移演化的物理规律，表明模型能够合理地量化预测的不确定性。

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/chapter3/uncertainty_comparison.pdf}
\caption{LSTM与GRU模型预测不确定性对比}
\label{fig:uncertainty-comparison}
\end{figure}

综合来看，LSTM和GRU在滑坡位移预测任务上表现相当。GRU的参数量约为LSTM的75\%，训练速度略快，但在预测精度和不确定性量化方面与LSTM无显著差异。实际应用中可根据具体需求选择：追求极致性能可尝试LSTM，追求简洁高效可选择GRU。

\subsection{本章小结}

本章针对水库滑坡单点位移预测问题，提出了基于LSTM和GRU的概率预测方法。主要研究工作和结论如下：

（1）引入LSTM和GRU两种主流循环神经网络，详细阐述了其原理和数学公式，分析了其在时序预测中的优势。LSTM通过门控机制捕捉长期依赖，GRU作为LSTM的简化版本参数更少，两者均适合复杂时序预测任务。
...
（3）在藕塘滑坡MJ9监测点上进行了系统验证，LSTM模型取得了良好的预测性能，测试集$R^2$达到0.9139，RMSE约4.99 mm，相对误差约1\%。GRU模型性能与LSTM相当，平均预测值相差不到3 mm，预测不确定性分布相似。

（4）系统对比了LSTM和GRU的预测性能、不确定性和计算效率。结果表明两者性能相当，均可作为滑坡位移预测的可靠工具。GRU参数量约为LSTM的75\%，训练速度略快，但在预测精度方面无显著差异。实际应用中可根据具体需求选择。

（5）50次运行集成方法能够可靠地量化预测不确定性，平均标准差约400 mm，反映了模型的不确定性。标准差随时间变化，在位移变化剧烈时期不确定性增大，符合实际物理规律。
