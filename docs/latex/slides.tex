...
\begin{frame}{预警时序——MJ1 监测点}
  \centering
  \includegraphics[width=\textwidth,height=0.88\textheight,keepaspectratio]{figures/chapter4/warning_timeseries.pdf}
\end{frame}
...
\begin{frame}{预警归因——SHAP 可解释性分析}
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      ...
      \end{table}
      \vspace{0.2em}
      {\small 位移累积 + 库水位 + 地下水位合计 95.9\%，定量复现"主汛末蓄水→位移加速"机理}
    \end{column}
    \begin{column}{0.51\textwidth}
      \includegraphics[width=\textwidth,height=0.75\textheight,keepaspectratio]{figures/chapter4/shap_waterfall.pdf}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{主要结论与创新点}
  ...
  \begin{enumerate}
    ...
    \item \textbf{概率预警}：基于 $V_0 = 1.5\bar V + 2\sigma$ 的四级动态预警体系，各监测点独立确定阈值；测试集三点均 100\% 特异性、零误报；训练期阶跃段演示中 MJ1 与 MJ3 分别触发 70 天与 129 天黄色预警
  \end{enumerate}
  \vspace{-0.2em}
  \begin{alertblock}{创新点}
    \begin{enumerate}
      ...
      \item 基于 $V_0$ 内生阈值的四级动态概率预警：从滞后性向前瞻性跨越
    \end{enumerate}
  \end{alertblock}
  \vspace{-0.3em}
  {\footnotesize\centering 展望：迁移学习缓解数据稀缺 · 多案例扩展验证 · 物理信息神经网络（PINN）深度融合}
\end{frame}
