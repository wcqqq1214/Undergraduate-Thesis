...
\begin{frame}{预测结果}
  ...
  \begin{itemize}
    ...
    \item 三点变异系数 $<1\%$（MJ9 0.72\% / MJ1 0.62\% / MJ3 0.35\%），标准差占位移基线 $<1\%$
    \item 50次集成成功量化认知不确定性，模型稳定可复现
  \end{itemize}
\end{frame}
...
\begin{frame}{预警归因——SHAP 可解释性分析}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        \item 50次LSTM集成月速率中位数 36.2 mm/月，超过 $V_0=20.50$
    ...
    \end{column}
    \begin{column}{0.48\textwidth}
      \includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{figures/chapter4/shap_waterfall.pdf}
    \end{column}
  \end{columns}
  \vspace{0.3em}
  \begin{block}{}
    \centering
    位移累积 + 库水位 + 地下水位合计贡献 95.9\%，定量复现"主汛末蓄水→位移加速"机理
  \end{block}
\end{frame}
...
\begin{frame}{主要结论与创新点}
  ...
  \end{alertblock}
  \vspace{0.2em}
  {\small\centering 展望：迁移学习缓解数据稀缺 · 多案例扩展验证 · 物理信息神经网络（PINN）深度融合}
\end{frame}
